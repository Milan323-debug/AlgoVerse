...
export const sendResetPasswordEmail = async (email, username, otp) => {
    const hasBrevoKey = process.env.BREVO_API_KEY && process.env.BREVO_SENDER_EMAIL;

    if (!hasBrevoKey) {
        console.log("\n=======================================================");
        console.log(`[DEV MAIL] Reset Password Email Sent To: ${email}`);
        console.log(`[DEV MAIL] Username: ${username}`);
        console.log(`[DEV MAIL] OTP Code: ${otp}`);
        console.log("=======================================================\n");
        return;
    ...
    const htmlContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #333; border-radius: 10px; background-color: #060816; color: #ffffff;">
            ...
            <hr style="border: 0; height: 1px; background: rgba(255,255,255,0.1); margin-bottom: 25px;" />
            <p style="font-size: 16px; color: rgba(255,255,255,0.9);">Hello <strong>${username}</strong>,</p>
            <p style="font-size: 16px; color: rgba(255,255,255,0.9);">You have requested to reset your password. Please use the 6-digit password reset code (OTP) below to set a new password:</p>
            <div style="text-align: center; margin: 35px 0;">
                <span style="font-size: 34px; font-weight: bold; letter-spacing: 6px; color: #06B6D4; background: rgba(6,182,212,0.15); padding: 12px 24px; border-radius: 8px; border: 1px solid rgba(6,182,212,0.3); display: inline-block;">${otp}</span>
            </div>
            <p style="font-size: 14px; color: rgba(255,255,255,0.5);">This password reset code is valid for <strong>10 minutes</strong>. If you did not request this, please ignore this email and your password will remain unchanged.</p>
            <hr style="border: 0; height: 1px; background: rgba(255,255,255,0.1); margin-top: 30px; margin-bottom: 20px;" />
            <p style="font-size: 11px; color: rgba(255,255,255,0.3); text-align: center; margin: 0;">© ${new Date().getFullYear()} AlgoVerse. All rights reserved.</p>
        </div>
    `;
    ...
    try {
        ...
        console.log(`Password reset email sent to ${email} (messageId: ${data.messageId || 'unknown'})`);
    } catch (error) {
